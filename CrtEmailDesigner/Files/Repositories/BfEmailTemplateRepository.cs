...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	internal class BfEmailTemplateRepository : IBfEmailTemplateRepository
	{
		...
		private Entity CreateInternal(Guid emailId, Guid languageId) {
			Entity entity = _userConnection.EntitySchemaManager.GetInstanceByName(TemplateSchemaName)
				.CreateEntity(_userConnection);
			entity.SetDefColumnValues();
			entity.SetColumnValue("EmailId", emailId);
			entity.SetColumnValue("TemplateLanguageId", languageId);
			entity.Save();
			return entity;
		}
        ...
        /// <summary>
        /// Temporary method to create default template retaining the legacy convention of id equal to emailId.
        /// </summary>
        /// <param name="emailId"></param>
        /// <param name="languageId"></param>
        /// <returns></returns>
        internal EmailDesignerTemplateModel CreateLegacy(Guid emailId, Guid languageId) {
			Entity entity = _userConnection.EntitySchemaManager.GetInstanceByName(TemplateSchemaName)
				.CreateEntity(_userConnection);
			entity.SetDefColumnValues();
			entity.SetColumnValue("Id", emailId);
			entity.SetColumnValue("EmailId", emailId);
			entity.SetColumnValue("TemplateLanguageId", languageId);
			entity.Save();
		...
		public EmailDesignerTemplateModel Create(Guid emailId, Guid languageId) {
			Entity entity = CreateInternal(emailId, languageId);
			return FetchById(entity.PrimaryColumnValue);
		}

		/// <inheritdoc cref="IBfEmailTemplateRepository.CreateMany"/>
		public IList<EmailDesignerTemplateModel> CreateMany(Guid emailId, IEnumerable<Guid> languageIds) {
			var resultIds = new List<Guid>();
			foreach (Guid id in languageIds) {
				Entity entity = CreateInternal(emailId, id);
				resultIds.Add(entity.PrimaryColumnValue);
			}
